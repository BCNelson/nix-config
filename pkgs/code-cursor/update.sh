...
#
# Rewrites ./sources.json from Cursor's stable update channel. Adapted from
# nixpkgs' pkgs/by-name/co/code-cursor/update.sh, minus darwin (see
# ./default.nix for why we only carry linux).
#
# Run it directly -- `./update.sh` -- then rebuild. Nothing calls it on a
# schedule: this package only exists while nixpkgs trails the stable channel,
# and the overlay hands `code-cursor` back to nixpkgs once it catches up.
set -euo pipefail
...
echo "Updated sources.json: $CURRENT -> $VERSION"
